...
public class PatrolEnemy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_movingLeft)
            ...
            Debug.DrawLine(_boxCollider.bounds.max + new Vector3(_boxCollider.bounds.extents.x * 2,0f,0f), 
    ...
    {
        if(_movingLeft)
            _endOfPlatformRay = Physics2D.Linecast(_boxCollider.bounds.min + _boxCollider., _boxCollider.bounds.min - new Vector3(-_rayDistance, -_rayDistance), _groundMask);
        else
            _endOfPlatformRay = Physics2D.Linecast(_boxCollider.bounds.max + new Vector3(_boxCollider.bounds.extents.x * 2, 0f, 0f),
